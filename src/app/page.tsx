...
import { Sparkles, Image as ImageIcon, Loader2, AlertTriangle, Send, ListChecks, Info, MessageSquareText, Type, Download, Link as LinkIcon, Palette, PlusCircle, PlayCircle } from 'lucide-react';
...
const ImageGeneratorApp = () => {
  ...
  useEffect(() => {
    ...
    if (currentJob && currentJob.status === 'pending') {
      (async () => {
        // Update status in displayList first for immediate feedback
        ...
        // Update status in processingJobs (though this specific job is about to be processed)
    ...
    } else if (currentJob) { // If job is not pending (e.g. already processed, skipped), move to next
        setCurrentJobIndexInQueue(prevIndex => (prevIndex !== null ? prevIndex + 1 : 0));
    }
  ...
  const handleAddItemToQueue = () => {
    ...
    const uniqueId = Date.now() + displayList.length + Math.random(); // More robust unique ID
    ...
    switch (inputMode) {
      ...
      case 'title_prompt':
        ...
        // Add title as separate item if present
        if (titleInput.trim()) {
          setDisplayList(prev => [...prev, { id: uniqueId, type: 'title_comment', text: titleInput.trim() }]);
        }
        // Then, if prompt is present, prepare it for potential job
        if (promptForTitleInput.trim()) {
          ...
          newJobToAdd = {
            id: titleInput.trim() ? uniqueId + 0.1 : uniqueId, // Adjust ID if title was also added
          ...
          newItemToAdd = newJobToAdd; // The prompt job is also the display item
        } else if (!titleInput.trim()) { // No title and no prompt for title_prompt mode
            return;
        }
        break;
    ...
    if (newItemToAdd) {
        // If only a title was added in 'title_prompt' mode, newJobToAdd would be null.
        // newItemToAdd would have been set by the direct setDisplayList for the title.
        // We only add to displayList here if newItemToAdd was for a URL, skipped_prompt or an actual AI prompt.
  ...
  const handleStartBatchProcessing = () => {
    ...
    setCurrentJobIndexInQueue(0); // Start processing from the first job in the `processingJobs` array
  };
          ...
          <main>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              ...
              <Button
                ...
                className="w-full text-white font-semibold py-3 px-4 text-lg focus:ring-4 focus:ring-primary/50 disabled:bg-muted disabled:text-muted-foreground disabled:cursor-not-allowed hover:bg-pink-600/90 bg-pink-500" // Changed color slightly for distinction
              >
